	...
	public Vector<String> processInput(String msg) {
		...
		case NO_CONN:
			if (msg == null){
				...
				return vectorFromString("220 Service ready for new user.");
			}else{
		...
		case READY:
			if (msg != null && Pattern.matches("^QUIT\\s?", msg)){
				...
				return vectorFromString("331 User name ok, need password.");
			}else if(msg != null && Pattern.matches("^USER\\s?", msg)){
				return vectorFromString("530 Not logged in.");
			}else if(msg != null && Pattern.matches("^USER.*", msg)){
				return vectorFromString("501 Syntax error in parameters or arguments");
			}else{
				return vectorFromString("332 Need account for login.");
			}
		case USER:
			if (msg != null && Pattern.matches("^PASS (\\S)+$", msg)){
				connectionState = LOGGED_IN;
				return vectorFromString("230 User logged in.");
		...
		case LOGGED_IN:
			if (msg != null && Pattern.matches("^QUIT\\s?", msg)){
				...
				cmdTryCnt++;
				return vectorFromString("502 Command not implemented.");
			}else{
				connectionState = KILLED;
				return vectorFromString("221 Service closing control connection.");
			}
